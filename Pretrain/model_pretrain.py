...
#        torch.nn.init.normal_(self.cls_token, std=.02)
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        mask_for_patch1 = mask.reshape(-1, 6, 6, 6).unsqueeze(-1).repeat(1, 1, 1, 1, 64).reshape(-1, 6, 6, 6, 4, 4, 4).permute(0, 1, 4, 2, 5, 3, 6).reshape(1, 24, 24, 24).unsqueeze(1)
        # print(f"mask_for_patch1.shape: {mask_for_patch1.shape}")
        mask_for_patch2 = mask.reshape(-1, 6, 6, 6).unsqueeze(-1).repeat(1, 1, 1, 1, 8).reshape(-1, 6, 6, 6, 2, 2, 2).permute(0, 1, 4, 2, 5, 3, 6).reshape(1, 12, 12, 12).unsqueeze(1)
