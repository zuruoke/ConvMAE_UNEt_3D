...
weight = torch.load(f"{args.root_dir}/data/model_bestValRMSE.pt")
...
def train(global_step, train_loader, dice_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (global_step % eval_num == 0 and global_step != 0) or global_step == max_iterations:
            ...
            save_to_json(epoch_loss_values, f"{args.root_dir}/swinunetr_results", 'epoch_loss_values')
            save_to_json(metric_values, f"{args.root_dir}/swinunetr_results", 'metric_values')
            if dice_val > dice_val_best:
                dice_val_best = dice_val
                global_step_best = global_step
                torch.save(model.state_dict(), os.path.join(f"{args.root_dir}/swinunetr_results", "best_metric_model.pth"))
                print(
